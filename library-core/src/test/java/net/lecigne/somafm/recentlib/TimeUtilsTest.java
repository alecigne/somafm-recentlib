...
class TimeUtilsTest {
  ...
  void should_work(
      // Given
      Instant snapshotTimeUtc, ZoneId broadcastLocation, LocalTime broadcastTime, Instant expectedUtc
  ) {
    // When
    Instant utc = TimeUtils.broadcastTimeToInstant(broadcastTime, snapshotTimeUtc, broadcastLocation);
  ...
  /**
   * A track snapshotted on [1] on a radio located in [2] and displaying a local broadcasting time
   * of [3] was played on [4] (UTC time).
   */
